...
using AllowanceTracker.Functions.Extensions;

namespace AllowanceTracker.Functions.Helpers;

/// <summary>
/// Helper for JWT authorization in Azure Functions
/// </summary>
public class AuthorizationHelper
{
    ...
    /// </summary>
    public ClaimsPrincipal? ValidateToken(HttpRequestData req)
    {
        try
        {
    ...
    /// </summary>
    public async Task<(bool IsAuthorized, ClaimsPrincipal? Principal, HttpResponseData? ErrorResponse)> CheckAuthorizationAsync(
        HttpRequestData req,
        string[]? requiredRoles = null)
    {
        var principal = ValidateToken(req);

        if (principal == null)
        {
            var response = await req.CreateUnauthorizedResponseAsync("Valid JWT token required");
        ...
        if (requiredRoles != null && requiredRoles.Length > 0)
        {
            ...
            {
                var response = await req.CreateForbiddenResponseAsync($"User must have one of the following roles: {string.Join(", ", requiredRoles)}");
